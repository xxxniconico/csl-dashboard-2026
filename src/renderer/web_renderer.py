...
<body class="bg-midnight text-slate-100 min-h-screen pb-[max(4.5rem,calc(3.75rem+env(safe-area-inset-bottom,0px)))] lg:pb-0">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:gap-5">
              ...
              <div id="matchCalendarPanel" class="shrink-0 rounded-xl border border-slate-700 bg-slate-900/45 p-2 sm:p-2.5 lg:order-2 lg:w-[min(100%,320px)]">
                <div class="mb-1.5 flex items-center justify-between gap-1.5">
                  <button type="button" id="matchCalPrev" class="min-h-[36px] min-w-[36px] rounded-md border border-slate-600 bg-slate-800/90 text-base leading-none text-slate-200 hover:border-accent/40 hover:text-accent sm:min-h-0 sm:min-w-0 sm:rounded-lg sm:px-2 sm:py-1 sm:text-lg" aria-label="上两周">‹</button>
                  <div id="matchCalTitle" class="min-w-0 flex-1 text-center text-xs font-semibold leading-tight text-slate-100 sm:text-sm"></div>
                  <button type="button" id="matchCalNext" class="min-h-[36px] min-w-[36px] rounded-md border border-slate-600 bg-slate-800/90 text-base leading-none text-slate-200 hover:border-accent/40 hover:text-accent sm:min-h-0 sm:min-w-0 sm:rounded-lg sm:px-2 sm:py-1 sm:text-lg" aria-label="下两周">›</button>
                </div>
                ...
                <div id="matchCalCells" class="mt-0.5 grid grid-cols-7 grid-rows-2 gap-0.5 sm:gap-1"></div>
                <p class="mt-1.5 text-sm leading-snug text-slate-500 sm:hidden">提示：日历为连续两周（自周日）；点日期看单场；赛程列表在下方。</p>
                <p class="mt-1.5 hidden text-sm leading-relaxed text-slate-500 sm:block sm:text-base">全部赛程：以系统当日为界，展示「早于今日最近一个有赛日」与「晚于今日最近一个有赛日」的全部场次。选俱乐部：该队已赛与未赛全景，并定位最近一场已赛。日历可切换上/下两周。</p>
  ...
  <script>
    ...
    /** 赛程日历双周窗口起始（周日 YYYY-MM-DD），共 14 格 */
    ...
    function matchInCalendarWindow(m, windowStartYmd){
      const k = matchDateKey(m);
      if (k.length !== 10 || !windowStartYmd) return false;
      const endY = addDaysYmd(windowStartYmd, 13);
      return k >= windowStartYmd && k <= endY;
    }

    function formatFortnightTitle(startYmd, endYmd){
    ...
    function renderMatchCalendar(matchDatesSet){
      ...
      const endYmd = addDaysYmd(startYmd, 13);
      matchCalTitleEl.textContent = formatFortnightTitle(startYmd, endYmd);
    ...
    function renderMatches(){
      ...
      for (const m of matches){
        const k = matchDateKey(m);
        if (k.length === 10 && matchInCalendarWindow(m, calendarWindowStartYmd)){
          matchDatesSet.add(k);
        }
      }
      ...
      const windowMatches = matches.filter(m => matchInCalendarWindow(m, calendarWindowStartYmd));
      let displayMatches;
      ...
      if (selectedMatchDate && !displayMatches.length){
        matchListEl.innerHTML = '<div class="rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-base text-slate-400">该日暂无比赛。可换一天或点击「显示本两周全部场次」。</div>';
        return;
      }

      if (!selectedMatchDate && scheduleViewMode === "month" && !windowMatches.length){
        matchListEl.innerHTML = '<div class="rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-base text-slate-400">当前两周暂无比赛（可切换上/下两周查看其他日期）。</div>';
        return;
      }
    ...
    if (matchCalPrevBtn) matchCalPrevBtn.addEventListener("click", () => {
      ...
      calendarWindowStartYmd = addDaysYmd(cur, -14);
      renderMatches();
    });
