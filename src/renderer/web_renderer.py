...
<head>
  ...
  <style>
    ...
    /*
     * 积分榜横向滚动：勿用 touch-action: pan-x —— 会禁止该区域的纵向滑动，
     * 导致整页无法上下滚动、赛程页月历滚不到视区内。
     */
    .touch-pan-x {
        ...
        <section id="view-matches" class="view-panel hidden">
          <div class="rounded-xl border border-slate-800 bg-surface/90 p-2.5 sm:rounded-2xl sm:p-4">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:gap-5">
              <div id="matchListWrap" class="order-2 min-w-0 lg:order-none lg:min-h-0 lg:flex-1">
                <div id="matchList" class="max-h-[min(42dvh,calc(100dvh-18rem))] overflow-auto pr-0.5 sm:max-h-[min(720px,70vh)] sm:pr-1"></div>
              </div>
              <div id="matchCalendarPanel" class="order-1 shrink-0 rounded-xl border border-slate-700 bg-slate-900/45 p-2.5 sm:p-3 lg:order-none lg:w-[min(100%,360px)]">
                ...
                <p class="mt-2 text-[10px] leading-snug text-slate-500 sm:hidden">提示：以今日为界展示相邻两个有赛日；点日期看单场；赛程列表在下方。</p>
