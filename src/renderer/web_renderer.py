          ...
          <div class="standings-table-scroll overflow-x-auto overscroll-x-contain -mx-1 rounded-lg px-1 sm:mx-0 sm:px-0">
            <table class="min-w-[720px] w-full text-[11px] sm:min-w-0 sm:text-sm">
              <thead class="bg-slate-900/60 text-xs text-slate-300">
                <tr>
                  <th class="sticky left-0 z-20 w-8 bg-slate-900/95 px-0.5 py-2 text-center font-semibold shadow-[2px_0_8px_rgba(0,0,0,0.3)] sm:w-auto sm:px-2">#</th>
                  <th class="sticky left-8 z-20 max-w-[5.5rem] bg-slate-900/95 px-0.5 py-2 text-left font-semibold shadow-[2px_0_8px_rgba(0,0,0,0.3)] sm:left-8 sm:max-w-none sm:px-2">球队</th>
        ...
        <section id="view-matches" class="view-panel hidden">
          <div class="rounded-xl border border-slate-800 bg-surface/90 p-2.5 sm:rounded-2xl sm:p-4">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:gap-5">
              ...
              <div id="matchCalendarPanel" class="shrink-0 rounded-xl border border-slate-700 bg-slate-900/45 p-2.5 sm:p-3 lg:order-2 lg:w-[min(100%,360px)]">
                <div class="mb-2 flex items-center justify-between gap-2">
                  <button type="button" id="matchCalPrev" class="min-h-[40px] min-w-[40px] rounded-lg border border-slate-600 bg-slate-800/90 text-lg leading-none text-slate-200 hover:border-accent/40 hover:text-accent sm:min-h-0 sm:min-w-0 sm:px-2 sm:py-1" aria-label="上一月">‹</button>
                  <div id="matchCalTitle" class="min-w-0 flex-1 text-center text-sm font-semibold text-slate-100"></div>
                  <button type="button" id="matchCalNext" class="min-h-[40px] min-w-[40px] rounded-lg border border-slate-600 bg-slate-800/90 text-lg leading-none text-slate-200 hover:border-accent/40 hover:text-accent sm:min-h-0 sm:min-w-0 sm:px-2 sm:py-1" aria-label="下一月">›</button>
                </div>
                <div class="grid grid-cols-7 gap-0.5 text-center text-xs font-medium uppercase tracking-wide text-slate-500">
                  <span>日</span><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span>六</span>
                </div>
                <div id="matchCalCells" class="mt-1 grid grid-cols-7 gap-1"></div>
                <p class="mt-2 text-xs leading-snug text-slate-500 sm:hidden">提示：以今日为界展示相邻两个有赛日；点日期看单场；赛程列表在下方。</p>
                <p class="mt-2 hidden text-xs leading-relaxed text-slate-500 sm:block sm:text-sm">全部赛程：以系统当日为界，展示「早于今日最近一个有赛日」与「晚于今日最近一个有赛日」的全部场次。选俱乐部：该队已赛与未赛全景，并定位最近一场已赛。换月/点某日规则不变。</p>
  ...
  <script>
      ...
      standingsBodyEl.innerHTML = standings.map((row, idx) => {
        ...
        return `
          <tr class="border-b border-slate-800 hover:bg-slate-800/50">
            <td class="sticky left-0 z-10 w-8 bg-slate-900/95 px-0.5 py-1.5 text-center text-[11px] font-medium tabular-nums shadow-[2px_0_8px_rgba(0,0,0,0.2)] sm:w-auto sm:px-2 sm:py-2 sm:text-sm">${idx + 1}</td>
            <td class="sticky left-8 z-10 max-w-[5.5rem] bg-slate-900/95 px-0.5 py-1.5 sm:left-8 sm:max-w-none sm:px-2 sm:py-2">
              <div class="flex items-center gap-0.5 sm:gap-2">
                ${clubImg(row.club_name, true)}
                <button type="button" data-club-name="${esc(row.club_name)}" class="club-filter-btn min-h-[40px] min-w-0 max-w-[5.5rem] flex-1 truncate rounded px-0.5 py-1 text-left text-[11px] leading-tight text-accent hover:bg-accent/10 sm:min-h-0 sm:max-w-none sm:px-1 sm:text-sm sm:leading-normal hover:underline" title="${esc(row.club_name)}">
                  ${esc(row.club_name)}
                </button>
              </div>
            </td>
    ...
    function renderMatchCalendar(matchDatesSet){
      ...
      for (let i = 0; i < startPad; i++){
        cells.push('<div class="min-h-[2.25rem]" aria-hidden="true"></div>');
      }
