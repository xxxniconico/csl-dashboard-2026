...
<body class="bg-midnight text-slate-100 min-h-screen pb-[max(4.5rem,calc(3.75rem+env(safe-area-inset-bottom,0px)))] lg:pb-0">
      ...
      <main class="space-y-4">
        <section id="view-standings" class="view-panel rounded-xl border border-slate-800 bg-surface/90 p-2.5 sm:rounded-2xl sm:p-4">
          ...
          <div class="standings-table-scroll overflow-x-auto overscroll-x-contain -mx-1 rounded-lg px-1 sm:mx-0 sm:px-0">
            <table class="min-w-[720px] w-full text-xs sm:min-w-0 sm:text-sm md:text-base">
        ...
        <section id="view-matches" class="view-panel hidden">
          <div class="rounded-xl border border-slate-800 bg-surface/90 p-2.5 sm:rounded-2xl sm:p-4">
            ...
            <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
              ...
              <select id="matchClubFilter" class="min-h-[40px] w-full max-w-lg rounded-lg border border-slate-600 bg-slate-800/90 px-3 py-2 text-base text-slate-100 shadow-inner sm:min-w-[220px]">
                <option value="">全部俱乐部</option>
              </select>
              <button id="clearClubFilterBtn" type="button" class="hidden min-h-[40px] rounded-md border border-slate-700 bg-slate-800 px-3 py-2 text-sm hover:border-accent/40 sm:min-h-0 sm:py-1">
                清除筛选
              </button>
            </div>
            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:gap-5">
              ...
              <div id="matchCalendarPanel" class="shrink-0 rounded-xl border border-slate-700 bg-slate-900/45 p-2 sm:p-2.5 lg:order-2 lg:w-[min(100%,320px)]">
                <div class="mb-1.5 flex items-center justify-between gap-1.5">
                  ...
                  <div id="matchCalTitle" class="min-w-0 flex-1 text-center text-sm font-semibold text-slate-100 sm:text-base"></div>
                ...
                <p class="mt-1.5 text-sm leading-snug text-slate-500 sm:hidden">提示：以今日为界展示相邻两个有赛日；点日期看单场；赛程列表在下方。</p>
                <p class="mt-1.5 hidden text-sm leading-relaxed text-slate-500 sm:block sm:text-base">全部赛程：以系统当日为界，展示「早于今日最近一个有赛日」与「晚于今日最近一个有赛日」的全部场次。选俱乐部：该队已赛与未赛全景，并定位最近一场已赛。换月/点某日规则不变。</p>
  ...
  <script>
    ...
    function renderLeagueNav(){
      ...
      const chip = `<div class="w-full rounded-lg border border-accent/50 bg-accent/15 px-3 py-2 text-left text-base font-semibold text-accent">${esc(league.name || "中超联赛")}</div>`;
    ...
    function renderMatchCalendar(matchDatesSet){
      ...
      for (let d = 1; d <= lastDay; d++){
        ...
        let cls = "relative flex min-h-[1.75rem] flex-col items-center justify-center rounded-md border text-xs font-medium transition-colors sm:min-h-[2rem] sm:rounded-lg sm:text-sm";
    ...
    function renderMatches(){
      ...
      if (selectedMatchDate && !displayMatches.length){
        matchListEl.innerHTML = '<div class="rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-base text-slate-400">该日暂无比赛。可换一天或点击「显示整月赛程」。</div>';
        return;
      }

      if (!selectedMatchDate && scheduleViewMode === "month" && !monthMatches.length){
        matchListEl.innerHTML = '<div class="rounded-lg border border-slate-700 bg-slate-900/60 p-3 text-base text-slate-400">当前月份暂无比赛（可切换月份查看其他赛程）。</div>';
        return;
      }
