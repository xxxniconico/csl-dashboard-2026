...
<body class="bg-midnight text-slate-100 min-h-screen touch-manipulation pb-[max(4.5rem,calc(3.75rem+env(safe-area-inset-bottom,0px)))] lg:pb-0">
      ...
      <main class="space-y-4">
        <section id="view-standings" class="view-panel rounded-xl border border-slate-800 bg-surface/90 p-2.5 sm:rounded-2xl sm:p-4">
          <div class="mb-2 flex flex-col gap-2 sm:mb-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            <h2 class="text-[15px] font-bold leading-snug sm:text-lg">积分榜</h2>
            <div class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:gap-2">
              <button type="button" id="sortOfficialBtn" class="sort-standings-btn min-h-[40px] rounded-lg border border-accent/50 bg-accent/15 px-2 py-2 text-[11px] font-semibold leading-tight text-accent sm:min-h-0 sm:px-3 sm:text-sm">
                <span class="sm:hidden">官方积分</span><span class="hidden sm:inline">按官方积分排序</span>
              </button>
              <button type="button" id="sortMatchBtn" class="sort-standings-btn min-h-[40px] rounded-lg border border-slate-600 bg-slate-800/80 px-2 py-2 text-[11px] font-semibold leading-tight text-slate-200 sm:min-h-0 sm:px-3 sm:text-sm">
                <span class="sm:hidden">赛场积分</span><span class="hidden sm:inline">按赛场积分排序</span>
              </button>
            </div>
          </div>
          <details id="officialPolicyDetails" class="mb-2 rounded-lg border border-slate-700 bg-slate-900/50 text-[11px] text-slate-400 sm:mb-3 sm:text-sm">
          ...
          <div class="touch-pan-x overflow-x-auto overscroll-x-contain -mx-1 rounded-lg px-1 sm:mx-0 sm:px-0">
            <table class="min-w-[520px] w-full text-[10px] sm:min-w-0 sm:text-sm">
              <thead class="bg-slate-900/60 text-[10px] text-slate-300 sm:text-xs">
                <tr>
                  ...
                  <th class="sticky left-7 z-20 max-w-[4.5rem] bg-slate-900/95 px-0.5 py-2 text-left font-semibold shadow-[2px_0_8px_rgba(0,0,0,0.3)] sm:left-8 sm:max-w-none sm:px-2">球队</th>
        ...
        <section id="view-matches" class="view-panel hidden">
          <div class="rounded-xl border border-slate-800 bg-surface/90 p-2.5 sm:rounded-2xl sm:p-4">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:gap-5">
              <div id="matchListWrap" class="order-1 min-w-0 flex-1 lg:order-none">
                <div id="matchList" class="max-h-[min(52dvh,calc(100dvh-14rem))] overflow-auto pr-0.5 sm:max-h-[min(720px,70vh)] sm:pr-1"></div>
              </div>
              <div id="matchCalendarPanel" class="order-2 shrink-0 rounded-xl border border-slate-700 bg-slate-900/45 p-2.5 sm:p-3 lg:order-none lg:w-[min(100%,360px)]">
                ...
                <div class="grid grid-cols-7 gap-0.5 text-center text-[10px] font-medium uppercase tracking-wide text-slate-500">
                  <span>日</span><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span>六</span>
                </div>
                <div id="matchCalCells" class="mt-1 grid grid-cols-7 gap-1"></div>
                <p class="mt-2 text-[10px] leading-snug text-slate-500 sm:hidden">提示：以今日为界展示相邻两个有赛日；可筛选俱乐部；点日期看单场。</p>
                <p class="mt-2 hidden text-[10px] leading-relaxed text-slate-500 sm:block">全部赛程：以系统当日为界，展示「早于今日最近一个有赛日」与「晚于今日最近一个有赛日」的全部场次。选俱乐部：该队已赛与未赛全景，并定位最近一场已赛。换月/点某日规则不变。</p>
        ...
        <section id="view-players" class="view-panel hidden space-y-2 sm:space-y-4">
          <div id="playersSubviewList" class="space-y-2 sm:space-y-4">
            <div class="rounded-xl border border-slate-800 bg-surface/90 p-2.5 sm:rounded-2xl sm:p-4">
              <h2 class="mb-1 text-[15px] font-bold leading-snug sm:mb-2 sm:text-lg">球员榜</h2>
              <p class="mb-2 text-[11px] leading-snug text-slate-500 sm:mb-3 sm:text-xs">筛选俱乐部后点球员名查看数据与注册档案</p>
              <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
  ...
  <nav class="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-800 bg-slate-900/98 backdrop-blur-md shadow-[0_-4px_20px_rgba(0,0,0,0.35)] lg:hidden" style="padding-bottom: env(safe-area-inset-bottom, 0px);">
    <div class="mx-auto flex max-w-[1600px]">
      <button type="button" data-view="standings" class="view-btn flex min-h-[54px] flex-1 flex-col items-center justify-center gap-0.5 border-t-[3px] border-transparent px-1 py-1.5 text-[11px] font-semibold tracking-tight text-slate-200 active:bg-slate-800/80">
        <span class="text-lg leading-none">📊</span>
        <span>积分榜</span>
      </button>
      <button type="button" data-view="matches" class="view-btn flex min-h-[54px] flex-1 flex-col items-center justify-center gap-0.5 border-t-[3px] border-transparent px-1 py-1.5 text-[11px] font-semibold tracking-tight text-slate-200 active:bg-slate-800/80">
        <span class="text-lg leading-none">⚽</span>
        <span>赛程</span>
      </button>
      <button type="button" data-view="players" class="view-btn flex min-h-[54px] flex-1 flex-col items-center justify-center gap-0.5 border-t-[3px] border-transparent px-1 py-1.5 text-[11px] font-semibold tracking-tight text-slate-200 active:bg-slate-800/80">
        <span class="text-lg leading-none">📈</span>
        <span>球员</span>
      </button>
    </div>
  ...
  <script>
    ...
    function clubImg(name, compact){
      ...
      if (!src){
        return `<span class="${box} rounded border border-slate-700 bg-slate-900 text-[9px] font-bold text-slate-500 sm:rounded-md sm:text-[10px] sm:text-xs" title="${esc(name)}">${esc(initial)}</span>`;
      }
      return `<span class="${box} overflow-hidden rounded border border-slate-700 bg-slate-900 sm:rounded-md">
        ...
        <span class="hidden h-full w-full items-center justify-center text-[9px] font-bold text-slate-500 sm:text-xs">${esc(initial)}</span>
      </span>`;
    }
      ...
      standingsBodyEl.innerHTML = standings.map((row, idx) => {
        ...
        return `
          <tr class="border-b border-slate-800 hover:bg-slate-800/50">
            <td class="sticky left-0 z-10 w-7 bg-slate-900/95 px-0.5 py-1.5 text-center text-[10px] font-medium tabular-nums shadow-[2px_0_8px_rgba(0,0,0,0.2)] sm:w-auto sm:px-2 sm:py-2 sm:text-sm">${idx + 1}</td>
            <td class="sticky left-7 z-10 max-w-[4.75rem] bg-slate-900/95 px-0.5 py-1.5 sm:left-8 sm:max-w-none sm:px-2 sm:py-2">
              <div class="flex items-center gap-0.5 sm:gap-2">
                ${clubImg(row.club_name, true)}
                <button type="button" data-club-name="${esc(row.club_name)}" class="club-filter-btn min-h-[36px] min-w-0 flex-1 truncate rounded px-0.5 py-0.5 text-left text-[10px] leading-snug text-accent hover:bg-accent/10 sm:min-h-0 sm:max-w-none sm:px-1 sm:text-sm sm:leading-normal hover:underline" title="${esc(row.club_name)}">
                  ${esc(row.club_name)}
                </button>
              </div>
            </td>
            <td class="px-0.5 py-1.5 text-center tabular-nums text-slate-200 sm:px-2 sm:py-2">${n(row.played)}</td>
            <td class="px-0.5 py-1.5 text-center sm:px-2 sm:py-2">
              <span class="font-mono text-[9px] tabular-nums leading-none text-slate-300 sm:hidden" title="胜-平-负">${n(wdl[0])}-${n(wdl[1])}-${n(wdl[2])}</span>
            ...
            <td class="px-0.5 py-1.5 text-center tabular-nums text-slate-200 sm:px-2 sm:py-2">${n(row?.summary?.goals_for)}</td>
            <td class="px-0.5 py-1.5 text-center tabular-nums text-slate-200 sm:px-2 sm:py-2">${n(row?.summary?.goals_against)}</td>
            <td class="px-0.5 py-1.5 text-center tabular-nums sm:px-2 sm:py-2 ${n(row.goal_difference) >= 0 ? "text-success" : "text-danger"}">${n(row.goal_difference)}</td>
            <td class="px-0.5 py-1.5 text-center font-semibold tabular-nums text-slate-200 sm:px-2 sm:py-2">${n(row.points)}</td>
            <td class="px-0.5 py-1.5 text-center tabular-nums sm:px-2 sm:py-2 ${n(row.penalty_points) > 0 ? "text-danger font-semibold" : "text-slate-300"}">-${n(row.penalty_points)}</td>
      ...
      return `
        <button type="button" data-match-index="${idx}" class="match-item mb-2 w-full rounded-lg border border-slate-700 bg-slate-900/60 p-2 text-left active:bg-slate-800/80 sm:p-3 sm:hover:border-accent/40">
          <div class="mb-1 flex flex-wrap items-center justify-between gap-1">
            <span class="min-w-0 flex-1 truncate text-[10px] text-slate-400 sm:text-xs">${esc(metaLine || "—")}</span>
            <span class="shrink-0 rounded px-1.5 py-0.5 text-[10px] ${sui.cls} sm:px-2 sm:text-xs">${esc(sui.label)}</span>
          </div>
          <div class="mb-1.5 truncate text-[10px] leading-snug text-slate-500 sm:text-xs">${stadiumTxt}</div>
          <div class="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-1.5 sm:gap-3">
            <div class="flex min-w-0 items-center justify-end gap-1 sm:gap-2">
              <span class="min-w-0 truncate text-right text-[11px] font-semibold leading-tight text-slate-100 sm:text-sm">${esc(m.home_club)}</span>
              ${clubImg(m.home_club)}
            </div>
            ...
            <div class="flex min-w-0 items-center justify-start gap-1 sm:gap-2">
              ${clubImg(m.away_club)}
              <span class="min-w-0 truncate text-left text-[11px] font-semibold leading-tight text-slate-100 sm:text-sm">${esc(m.away_club)}</span>
            </div>
          </div>
        </button>
      `;
