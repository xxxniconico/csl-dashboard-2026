...
<body class="bg-midnight text-slate-100 min-h-screen touch-manipulation pb-[max(4.5rem,calc(3.75rem+env(safe-area-inset-bottom,0px)))] lg:pb-0">
          ...
          <div class="rounded-2xl border border-slate-800 bg-surface/90 p-3 sm:p-4">
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:gap-5">
              <div id="matchCalendarPanel" class="shrink-0 rounded-xl border border-slate-700 bg-slate-900/45 p-3 lg:w-[min(100%,360px)]">
                ...
                <p class="mt-2 text-[10px] leading-relaxed text-slate-500">全部赛程：首次按系统日期锚定后，展示「前一有赛日+后一有赛日」当日全部比赛。选俱乐部：该队已赛与未赛全景，并定位最近一场已赛。换月/点某日规则不变。</p>
              ...
              <div id="matchListWrap" class="min-w-0 flex-1">
                <div id="matchList" class="max-h-[min(640px,calc(100dvh-11rem))] overflow-auto pr-1 sm:max-h-[min(720px,70vh)]"></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="view-players" class="view-panel hidden space-y-3 sm:space-y-4">
          <div id="playersSubviewList" class="space-y-3 sm:space-y-4">
            <div class="rounded-2xl border border-slate-800 bg-surface/90 p-3 sm:p-4">
              ...
              <p class="mb-3 text-[11px] text-slate-500 sm:text-xs">先按俱乐部筛选（可选），点击球员姓名查看赛季数据与中足联注册档案（二级页面）</p>
              <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
              ...
              <div id="playerList" class="max-h-[min(70dvh,calc(100dvh-12rem))] overflow-auto pr-1 sm:max-h-[min(720px,75vh)]"></div>
            </div>
          </div>
  ...
  <script>
    ...
    /**
     * 全体俱乐部、首次进入：以 nearestMatchDateYmd 在「全部有赛自然日」序列中的位置为锚，
     * 展示锚点的前一个有赛日 + 后一个有赛日的全部比赛；赛季首尾无「前」或「后」时改为展示相邻两日。
     */
    function buildAdjacentMatchDaysWindow(matches){
      ...
      let idx = days.indexOf(anchor);
      if (idx < 0){
        const t = todayYmd();
        const j = days.findIndex(d => d >= t);
        idx = j >= 0 ? j : days.length - 1;
      }
      const want = new Set();
      if (days.length === 1){
        want.add(days[0]);
        ...
        want.add(days[days.length - 2]);
        want.add(days[days.length - 1]);
      } else {
        want.add(days[idx - 1]);
        want.add(days[idx + 1]);
      }
      ...
      return `
        <button type="button" data-match-index="${idx}" class="match-item mb-2 w-full rounded-lg border border-slate-700 bg-slate-900/60 p-2.5 text-left active:bg-slate-800/80 sm:p-3 sm:hover:border-accent/40">
          <div class="mb-1 flex flex-wrap items-center justify-between gap-1">
            <span class="text-[11px] text-slate-400 sm:text-xs">${esc(metaLine || "—")}</span>
            <span class="rounded px-2 py-0.5 text-[10px] ${sui.cls} sm:text-xs">${esc(sui.label)}</span>
          </div>
    ...
    function renderMatches(){
      ...
      if (matchViewNeedsDateAnchor && matches.length){
        ...
        if (selectedClub){
          ...
          const pivot = nearestMatchDateYmd(matches);
